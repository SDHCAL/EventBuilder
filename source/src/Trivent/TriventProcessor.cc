...
void TriventProcessor::FillTimes()
{
  ...
  for (std::map<int,int>::iterator it=Times.begin(); it!= Times.end(); ++it)
  {
    ...
    if (!eraseIt) 
	  {
	    std::map<int,int>::iterator itnext=it; ++itnext;
    ...
    if (eraseIt)
	  {
	    std::map<int,int>::iterator itprev=it; --itprev;
  ...
  if (evtP != NULL)
  {
    ...
    for(unsigned int i=0; i< _hcalCollections.size(); i++)
    {
      ...
      { 
        ...
        while(fabs(middle->first-before->first)<=_timeWin && before!=RawHits.begin()) --before; ++before;
