...
StringParameters::StringParameters( const StringParameters& sp ){_map=sp._map;}
...
void StringParameters::erase( const std::string& key ) {_map.erase( key );}

bool StringParameters::isParameterSet(  const std::string& key ) {return _map[ key ].size() > 0 ;}
